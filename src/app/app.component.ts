      ...
      ">`;
        ...
        for (let i = 0; i < objects.length; i++) {
          ...
          if (o.type === "image") {
            ...
            transform: scale(${img.scaleX || 1}, ${img.scaleY || 1}) rotate(${
              img.angle || 0
            }deg);
            transform-origin: top left;
          "/>`;
        ...
        .map(([name, id]) => {
          const values =
            name === "White"
              ...
              : ["0", "0", "0", "1"];
          const colorSpace =
            name.startsWith("Web") || name === "White"
              ? colorSpaceRGB
              : colorSpaceCMYK;
          return `<color id="${id}"><name>${name}</name><colorSpace>${colorSpace}</colorSpace>${values
            .map((v) => `<values>${v}</values>`)
            .join(
              ""
            )}<spot>false</spot><autoName>false</autoName><overprint>false</overprint></color>`;
        })
        .join("")}
    </colors>
    <contexts>
